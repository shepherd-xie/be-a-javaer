...
![](/assets/import 2.1-5.png)通过内存的分析可以发现，在引用数据类型关系里面，一块没有任何栈内存指向的堆内存空间就将成为垃圾，所有的垃圾会不定期的被垃圾收集器（GC）进行回收，回收后会释放掉其所占用的空间。

虽然在Java里面支持了自动的垃圾收集处理，但是在代码的开发过程之中应该尽量的减少垃圾空间的产生。
